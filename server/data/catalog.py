# Real Mundial 2026 sticker catalog — canonical backend copy.
# Mirrors src/data/catalog.js (both ported from MundialSwap / Panini official CSV).
# build_catalog() yields the 994 cards in physical album order, used to seed the DB.

# -- Physical album order ------------------------------------------------------
# 00(1) -> FWC(19) -> Block1(24x20=480) -> CC(14) -> Block2(24x20=480) = 994 total
BLOCK1_CODES = [
...
# (section, count) tuples in physical order.
ALBUM_SECTIONS = (
    [("00", 1), ("FWC", 19)]
    + [(c, 20) for c in BLOCK1_CODES]
    + [("CC", 14)]
    + [(c, 20) for c in BLOCK2_CODES]
)


# -- Tier helpers --------------------------------------------------------------
# Tiers: ESP (especiales 00/FWC), CC (Coca-Cola), ESC (escudo), EQP (team photo), JUG (jugador)
def get_tier(section, pos_in_section):
    if section in ("00", "FWC"):
        return "ESP"
    if section == "CC":
        return "CC"
    if pos_in_section == 1:
        return "ESC"
    if pos_in_section == 13:
        return "EQP"
    return "JUG"
...
def get_rarity(tier):
    if tier == "ESP":
        return "legend"
    if tier in ("ESC", "EQP", "CC"):
        return "gold"
    return "base"


def get_category(tier):
    return {
        "ESP": "Especial",
        "ESC": "Escudo",
        "EQP": "Foto del equipo",
...
def build_catalog():
    """The complete 994-card album in physical order, ready to seed the DB.

    Returns a list of dicts: {code, number, name, team, category, rarity}.
    """
    cards = []
    number = 1
    for section, count in ALBUM_SECTIONS:
        for pos in range(1, count + 1):
            ...
            tier = get_tier(section, pos)
            cards.append({
                "code": code,
                "number": number,
                "name": STICKER_NAMES.get(code, code),
                "team": section,
